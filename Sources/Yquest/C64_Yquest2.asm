...
UserRam equ $8000	;Game Vars - Moved to avoid sprites
...
*=$0810	;Start at $0810
	...
	lda #<Sprites			;Source Bitmap data
	sta z_L
	lda #>Sprites
	sta z_H

	lda #<(Sprites_End-Sprites)	;Length of bitmap data
	sta z_C
	lda #>(Sprites_End-Sprites)
	sta z_B
	
	lda #<(ScrBase+$1000)	;Will only work when scrBase@ $4000 due to chrom
	sta z_E					;Sprite settings at $47F8-$47FF
	lda #>(ScrBase+$1000)	;Sprites at $5000-$5FFF, screen at $6000
	sta z_D
...
DoGetHSpriteObj:
	...
	pushpair z_iy
		ldy #O_SprNum
		lda (z_IX),y	;Sprite Source (0=Player 1=Dead player)
		asl
		asl				;Sprite (4 frames per sprite)
		sta z_h			
	
		lda SpriteFrame	;Show Sprite of Objec XY	
		clc 
		adc #64			;lda #($1000/64)=64		;Sprite Ram base 
		adc z_h
		...
		lda #%00011010	;Sprite Color 8 = Brown
		sta z_l
		...
		ldy #O_Xpos
		lda (z_IX),y	
		asl 			;stored Xpos is in pairs of pixels
		sta z_ixl
		lda #0
		adc #0
		sta z_ixh		;Add carry to top byte
		
		lda #0
		sta z_iyh
		addpair z_ix,22	;Offset for top left of screen
		addpair z_iy,46
DoGetHSpriteObj2:			
		lda z_as
		jsr SetHardwareSprite	;Actually set Hardware sprite
	pullpair z_iy
	pullpair z_ix
	pullpair z_bc
	rts	
...
; z_L=%-XY4CCCC	4=4color mode C= sprite color X=dblX Y=dblY
SetHardwareSprite:
		tay					;Sprite Number
		lda $D015
		...
		lda z_h
		sta ScrBase+$07F8,y	;Pointer ($47F8)
...
Bitmap:
	...
	;Only player is represented as a sprite
Sprites:
	incbin "\ResAll\Yquest\C64_YquestSprites.raw"
Sprites_End:
