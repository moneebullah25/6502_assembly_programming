...
;Example 3 - Immediate
	;clc						;Clear the Carry
	;lda #$A0				;LoaD the Accumulator with hex 10
	;jsr Monitor				;Show the monitor
...
;Example 4 - Zero Page / Direct page	
	;lda $80		; lda 128	;Load A from ZP address $80 = $0080
	;jsr Monitor				;Show the monitor
	;jmp *					;Inf Loop
	
;Example 5 - Zero Page Indexed X,Y
	;ldx #$1		;ldx #1		;Load X with 1
	...
	;jsr Monitor				;Show the monitor
	;ldy #$3					;Load Y with 3
	;lda $80, Y				;Load A from Zeropage $80 + Y (so load from Zeropage $83)
	;jsr Monitor				;Show the monitor
	;jmp *					;Inf Loop
...
;Example 6 - Absolute 
	...
	;ldx 8192				;Load from same address $2000 but in decimal
	;jsr Monitor				;Call the monitor
	;jmp *					;Inf Loop
...
; Example 9 - Preindexed Indirect X
	...
	;jsr Monitor				;Call the monitor
	;;ldy #1					;Load Y with 1
	;;lda ($80,Y) ; Doesn't work "missing closing parenthesis in addressing mode"
	;jsr Monitor
	;jmp *					;Inf Loop
